...
public class user {
    ...
    public user(int idUser, String cprNumber, String name, String surname, com.covidcenter.covidcenter.model.userType userType, String email, String loginTime, String password, String token) {
        this.idUser = idUser;
...
// Now we need to zero pad it if you actually want the full 32 chars.
    ...
    public boolean nameValid() throws Exception {
        ...
        if (name.length()<1){ throw new Exception("Field can not be empty");}
        return true; }
    public boolean surnameValid() throws Exception {
        if (surname==null) {
            throw new Exception("Field can not be empty");
        }
        if (surname.length()<1){ throw new Exception("Field can not be empty");}
        return true; }
    public boolean passValid() throws Exception {
        if (password==null) {
            throw new Exception("Field can not be empty");
        }
        if (password.length()<5){ throw new Exception("Minimum 6 characters");}
        return true; }
